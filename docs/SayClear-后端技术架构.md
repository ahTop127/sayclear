...
- 客户端是最后一道闸：空串、确认话术、明显未完成稿、截断残句，一律当失败，不填入。详见第 7 节。
...
| 网关 | `https://wintoken.dev/v1`（OpenAI 兼容；`cun.ai` 在本机连不上，已弃用） |
| 接口 | `POST /chat/completions` |
| 鉴权 | `Authorization: Bearer`，值只从 `.env` 读，**不写进代码** |
| 思考模式 | 第一期 **`thinking: { "type": "disabled" }`**。产品 HUD 已经在等；再开深度思考会把等待拉长。质量不够再改为开启 |
...
8. **失败宁可空：** 转写为空、明显没说完、或只有「对对对 / 好的」类确认话术时，不要编一段需求充数（客户端也会当失败，见 §5.8）。
...
- 只有确认话术或扮演前缀（「我理解你的需求是」等），剥掉前缀后若仍无实质正文也失败
